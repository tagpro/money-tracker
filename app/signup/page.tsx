...
function SignupForm() {
  ...
  useEffect(() => {
    ...
    if (invite) {
      ...
      // Check if it's the first user even without an invite code
      verifyInvite('');
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Mark invite as used (only if not first user)
        if (!isFirstUser) {
      ...
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-lg">
        <div>
          <h2 className="text-3xl font-bold text-center text-gray-900">
            {isFirstUser ? 'Initial Setup' : 'Create your account'}
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            {isFirstUser 
              ? 'You are the first user and will be the administrator' 
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {isFirstUser && (
            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded">
              ℹ️ First user registration: Invite code not required.
            </div>
          )}
